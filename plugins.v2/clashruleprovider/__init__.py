...
class ClashRuleProvider(_PluginBase):
    ...
    def rule_providers(self) -> Optional[Dict[str, Any]]:
        ...
        for key, value in self._clash_config.get("rule-providers", {}).items():
            if value.get("path", '').startswith("./CRP/"):
                continue
    ...
    def update_subscription_service(self) -> bool:
        """修复点：正确处理响应内容，避免 'str' object has no attribute 'get' 错误"""
        ...
        try:
            ...
            if isinstance(content, bytes):
                ...
                try:
                    ...
                    # 如果UTF-8解码失败，尝试Base64解码
                    try:
                        import base64
                        content = base64.b64decode(content).decode('utf-8')
                    except:
                        logger.error("订阅内容解码失败")
                        return False
            ...
            if not isinstance(rs, dict):
                logger.error(f"订阅内容解析后不是字典类型: {type(rs)}")
                return False
        ...
        if 'Subscription-Userinfo' in ret.headers:
            try:
                ...
                self._subscription_info['download'] = variables.get('download', 0)
                self._subscription_info['upload'] = variables.get('upload', 0)
                self._subscription_info['total'] = variables.get('total', 0)
                self._subscription_info['expire'] = variables.get('expire', 0)
            except Exception as e:
    ...
    def __remove_nodes_by_keywords(self, clash_config: Dict[str, Any]) -> Dict[str, Any]:
        """修复点：确保只处理字典类型，避免 'str' object has no attribute 'get' 错误"""
        if not isinstance(clash_config, dict):
            logger.error(f"配置不是字典类型: {type(clash_config)}")
            return {}
